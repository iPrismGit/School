...
class DayCareRepository(private var context: Context) {
    ...
    suspend fun fetchDayCareStatus(request: DayCareStatusApiRequest): DayCareStatusApiResponse {
        var response = apiService.fetchDaycareStatus(request)

        if (response.message.equals("Invalid or expired token", true)) {
            val refreshed = authRepository.refreshToken()
            if (refreshed) {
                response = apiService.fetchDaycareStatus(request)
            }
        }
