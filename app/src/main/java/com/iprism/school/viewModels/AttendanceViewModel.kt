...
import com.iprism.school.model.classteachermodel.AcademicYear
...
class AttendanceViewModel(private var repository: AttendanceRepository) : ViewModel() {
    ...
    private val _academicYearsResponse = MutableLiveData<UiState<List<AcademicYear>>>()
    val academicYearsResponse: LiveData<UiState<List<AcademicYear>>> = _academicYearsResponse
    ...
    fun fetchAcademicYears(request : ClassTeacherApiRequest) {
        viewModelScope.launch {
            ...
            try {
                ...
                if (response.status) {
                    _academicYearsResponse.value = UiState.Success(response.academicYearsResponse)
                } else {
